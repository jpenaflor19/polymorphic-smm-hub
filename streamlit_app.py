...
# --- WORKSPACE: GRAPHICS (WITH AUTO-GENERATION) ---
        ...
        st.header("🎨 Instant AI Graphic Creator")
        st.write("Describe your vision, and I'll generate the image right here.")
        
        desc = st.text_input("What visual do you need?", placeholder="e.g. A vibrant 3D social media icon for a travel brand")
        
        if st.button("Generate Image"):
            with st.spinner("Gemini is painting your masterpiece..."):
                try:
                    # Step 1: Enhance the prompt for best quality
                    prompt_enhancer = model.generate_content(f"Expand this into a professional image generation prompt: {desc}")
                    enhanced_prompt = prompt_enhancer.text
                    
                    # Step 2: Generate the image
                    # Note: We use the 'imagen' or 'imagen-3' model via the tool
                    image_result = genai.ImageGenerationModel("imagen-3").generate_images(
                    ...
                    st.image(image_result.images[0].url, caption="Generated by Gemini 3", use_column_width=True)
                    st.success("Image Ready! Right-click to save.")
                except Exception as e:
                    st.error("Image generation is restricted on some API keys or regions.")
                    st.info("Fallback: Here is your professional prompt to use in Canva/Midjourney:")
