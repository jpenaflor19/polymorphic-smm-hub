...
# --- WORKSPACE: GRAPHICS (WITH AUTO-GENERATION) ---
    elif workspace == "🎨 AI Graphic Designer":
        st.header("🎨 Professional Creative Briefs")
        st.write("Get high-end prompts optimized for Midjourney, DALL-E, and Canva.")
        
        desc = st.text_input("What visual do you need?", placeholder="e.g. A salesperson as a superhero")
        
        if st.button("Generate Pro Briefs"):
            with st.spinner("Designing your visual strategy..."):
                # System Prompt to ensure "WOW" output
                prompt = f"""
                Act as a world-class Art Director. For the concept '{desc}', provide:
                1. A 'Cinematic/Realistic' prompt for Midjourney.
                2. A '3D Render/Pixar' style prompt for social media.
                3. A 'Minimalist Vector' style prompt for infographics.
                Include aspect ratio suggestions (e.g., --ar 9:16 for Stories).
                """
                res = model.generate_content(prompt)
                
                # Using our Glassmorphism card to display it
                st.markdown(f'<div class="result-card">{res.text}</div>', unsafe_allow_html=True)
                st.info("💡 Copy these into any free image generator (like Microsoft Designer or Canva) for instant high-end results!")
